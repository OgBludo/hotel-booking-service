...
public class BookingServiceTests {
    ...
    void successFlow_confirmed() {
        ...
        Booking b = bookingService.createBooking(101L, 200L, LocalDate.now().plusDays(1), LocalDate.now().plusDays(3), "req-101");
    ...
    void failureFlow_cancelledWithCompensation() {
        ...
        Booking b = bookingService.createBooking(102L, 201L, LocalDate.now().plusDays(2), LocalDate.now().plusDays(4), "req-102");
        Assertions.assertEquals(Booking.Status.CANCELLED, b.getStatus());
    }

    @Test
    void timeoutFlow_cancelled() {
        stubFor(post(urlPathMatching("/rooms/\\d+/hold")).willReturn(aResponse().withFixedDelay(2500).withStatus(200)));
        stubFor(post(urlPathMatching("/rooms/\\d+/release")).willReturn(okJson("{}")));

        Booking b = bookingService.createBooking(103L, 202L, LocalDate.now().plusDays(3), LocalDate.now().plusDays(5), "req-103");
        Assertions.assertEquals(Booking.Status.CANCELLED, b.getStatus());
    }
    ...
    void idempotency_noDuplicate() {
        ...
        Booking b1 = bookingService.createBooking(104L, 203L, LocalDate.now().plusDays(4), LocalDate.now().plusDays(6), "req-104");
        Booking b2 = bookingService.createBooking(104L, 203L, LocalDate.now().plusDays(4), LocalDate.now().plusDays(6), "req-104");
